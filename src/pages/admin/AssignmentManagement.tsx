...
export const AssignmentManagement = () => {
  ...
  const [isExtractingPart, setIsExtractingPart] = useState<Record<string, boolean>>({});
  ...
  const handleImportPDFToPart = async (partKey: 'part1' | 'part2' | 'part3' | 'part4', file: File) => {
    ...
    try {
      ...
      setIndependentAssignmentFormData(prev => ({
        ...prev,
        [partKey]: (prev[partKey as keyof typeof prev] ? prev[partKey as keyof typeof prev] + '\n\n' : '') + extractedText
      }));
      
    } catch (error) {
      console.error(`Error extracting ${partKey}:`, error);
      alert('Có lỗi xảy ra khi trích xuất nội dung từ PDF.');
    } finally {
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  ...
                  <label className="flex items-center gap-1.5 px-3 py-1.5 bg-white border border-gray-200 text-gray-600 rounded-lg hover:bg-gray-50 transition-colors text-xs font-medium cursor-pointer">
                    {isGeneratingAssignment ? <Loader2 size={14} className="animate-spin" /> : <FileUp size={14} />}
