...
const SCHEMA = {
  'users': ['id', 'username', 'password', 'fullName', 'role', 'classId', 'dob'],
  ...
  'assignments': ['id', 'lessonId', 'title', 'description', 'dueDate', 'rubricJson'],
  'tests': ['id', 'title', 'topicId', 'durationMinutes', 'startTime', 'endTime', 'questions', 'assignedTo', 'createdAt'],
  'submissions': ['id', 'assignmentId', 'testId', 'studentId', 'content', 'submittedAt', 'score', 'feedback'],
  'progresses': ['id', 'studentId', 'lessonId', 'completed', 'completedAt'],
  'announcements': ['id', 'target', 'title', 'content', 'createdAt', 'authorId'],
  'bank_questions': ['id', 'subjectId', 'topicId', 'type', 'difficulty', 'content', 'options', 'optionsJson', 'correctAnswer', 'points', 'createdAt']
};
...
function fetchTableData(tableName) {
  ...
  return rows.map(row => {
    ...
    headers.forEach(header => {
      ...
      if (colIdx !== -1) {
        let val = row[colIdx];
        // Xử lý JSON string cho các trường đặc biệt
        if (['questions', 'assignedTo', 'content', 'interactiveContent', 'options', 'subQuestions', 'rubricJson', 'optionsJson'].includes(header) && typeof val === 'string' && (val.startsWith('{') || val.startsWith('['))) {
...
function setupDatabase() {
  ...
  if (typeof SpreadsheetApp.getUi === 'function') {
    try {
      SpreadsheetApp.getUi().alert('Khởi tạo cơ sở dữ liệu LMS thành công!');
    } catch (e) {
...
function getSeedData(tableName) {
  ...
  const seeds = {
    'users': [
      { id: 'u1', username: 'teacher', password: '123', fullName: 'Giáo viên Tin học', role: 'teacher', classId: '', dob: '1985-01-01' },
      { id: 'u2', username: 'student', password: '123', fullName: 'Học sinh Mẫu', role: 'student', classId: 'c1', dob: '2008-05-15' }
    ],
    'classes': [
    ...
    'assignments': [
      { id: 'a1', lessonId: 'l1', title: 'Bài tập về nhà Bài 1', description: 'Hãy tóm tắt nội dung chính của bài học.', dueDate: '2026-12-31' }
    ],
    'tests': [
      { 
    ...
    'bank_questions': [
      { 
        id: 'bq1', subjectId: 's1', topicId: 't1', type: 'multiple_choice', difficulty: 'recognition', 
        content: 'Đơn vị đo lượng thông tin nhỏ nhất là?', options: ['Byte', 'Bit', 'KB', 'MB'], 
        correctAnswer: 'Bit', points: 1, createdAt: now 
      }
    ]
  };
